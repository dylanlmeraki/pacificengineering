...
import AdminRoute from "@/components/internal/Routes/AdminRoute";
...
import ICPModal from "@/components/internal/Modals/ICPModal";
...
export default function SalesBotControl() {
  ...
  const { data: icpProfiles = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await base44.entities.ICPSettings.list('-created_date', 50);
      } catch (error) {
...
Return ONLY valid JSON:
{
  ...
  "emailsSent": ${mode === 'send' ? '[array of names]' : '[]'}
      ...
      for (const prospect of parsedResponse.prospectsFound || []) {
        try {
          const newProspect = await base44.entities.Prospect.create({
  ...
  const handleFollowUpRun = async () => {
    ...
    try {
      const prospects = await base44.entities.Prospect.filter({ status: "Contacted" });
      const outreachRecords = await base44.entities.SalesOutreach.list('-sent_date', 500);
      ...
      for (const prospect of prospects) {
        try {
          ...
          const followUpEmail = await base44.integrations.Core.InvokeLLM({
