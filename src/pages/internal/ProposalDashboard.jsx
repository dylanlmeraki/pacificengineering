...
export default function ProposalDashboard() {
  ...
  const sendProposalMutation = useMutation({
    mutationFn: async ({ id, data, proposal }) => {
      const updatedProposal = await base44.entities.Proposal.update(id, { 
        ...data, 
        status: 'sent',
      ...
      if (data.recipient_emails && data.recipient_emails.length > 0) {
        for (const email of data.recipient_emails) {
          try {
            const prospects = await base44.entities.Prospect.filter({ contact_email: email });
            if (prospects && prospects.length > 0) {
              await base44.entities.Prospect.update(prospects[0].id, {
                          ...
                          {sharingProposalId === proposal.id && (
                            <div className="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-200" onClick={(e) => e.stopPropagation()}>
                              ...
                              <div className="flex gap-2 mb-2">
                                ...
                                <Button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      await base44.functions.invoke('shareProposal', { proposalId: proposal.id, recipientEmail: shareEmail });
                              ...
                              <Button
                                ...
                                variant="outline"
                                onClick={() => {
                                  const url = `${window.location.origin}/ProposalDashboard?id=${proposal.id}`;
