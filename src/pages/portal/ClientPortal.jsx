...
import ClientDashboard from "@/components/portal/dashboard/ClientDashboard";
...
import SecureDocumentViewer from "@/components/portal/documents/SecureDocumentViewer";
import ProjectProgressTracker from "@/components/portal/projects/ProjectProgressTracker";
import ProjectCard from "@/components/portal/projects/ProjectCard";
import CommunicationHub from "@/components/portal/messaging/CommunicationHub";
import MessageThread from "@/components/portal/messaging/MessageThread";
import ProposalsList from "@/components/portal/proposals/ProposalsList";
import ClientProposalView from "@/components/portal/proposals/ClientProposalView";
import ClientAnalytics from "@/components/portal/analytics/ClientAnalytics";
import ClientInvoices from "@/components/portal/billing/ClientInvoices";
import ClientProfileSettings from "@/components/portal/profile/ClientProfileSettings";
...
export default function ClientPortal() {
  ...
  const { data: allMilestones = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const data = await Promise.all(
        ids.map(id => base44.entities.ProjectMilestone.filter({ project_id: id }))
      );
      return data.flat();
    },
  ...
  const { data: allChangeOrders = [] } = useQuery({
    ...
    queryFn: async () => {
      const ids = projects.map(p => p.id);
      const data = await Promise.all(
        ids.map(id => base44.entities.ChangeOrder.filter({ project_id: id }))
      );
      return data.flat();
    },
    enabled: projects.length > 0
  ...
  const { data: allDocuments = [] } = useQuery({
    ...
    queryFn: async () => {
      const ids = projects.map(p => p.id);
      const data = await Promise.all(
        ids.map(id => base44.entities.ProjectDocument.filter({ project_id: id }))
      );
      return data.flat().sort((a, b) => new Date(b.created_date) - new Date(a.created_date));
    },
    enabled: projects.length > 0
  });
  ...
  const { data: clientProposals = [] } = useQuery({
    ...
    queryFn: async () => {
      const ids = projects.map(p => p.id);
      const data = await Promise.all(
        ids.map(id => base44.entities.Proposal.filter({ project_id: id }))
      );
      return data.flat();
    },
    enabled: projects.length > 0
  ...
  const { data: myRequests = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.ProjectRequest.filter(
        { client_email: user.email },
        "-created_date"
      ),
    enabled: !!user
  ...
  const pendingChangeOrders = allChangeOrders.filter(c => c.status === "Pending Client Approval");
  const pendingProposals = clientProposals.filter(p =>
    ["sent", "viewed", "awaiting_signature"].includes(p.status)
  );
