...
export default function UserManagement() {
  ...
  const { data: auditLogs = [] } = useQuery({
    queryKey: ['audit-logs'],
    queryFn: async () => {
      return await base44.entities.AuditLog.list('-created_date', 200);
    },
    initialData: []
  });
  ...
  const { data: clientInvites = [] } = useQuery({
    ...
    queryFn: async () => {
      return await base44.entities.ClientInvite.list('-created_date', 100);
    },
    initialData: []
  });
  ...
  const inviteUserMutation = useMutation({
    mutationFn: async ({ email, name, role }) => {
      ...
      const inviteLink = `${window.location.origin}/auth?invite=true&email=${encodeURIComponent(email)}`;
  ...
  const copyInviteLink = (invite) => {
    const link = `${window.location.origin}/PortalRegister?token=${invite.invite_token}`;
